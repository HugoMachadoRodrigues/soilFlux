...
* **DESCRIPTION**: removed single quotes around acronyms (`CNN1D-PINN`, `SWRC`,
  `pF`) — CRAN policy requires single quotes only for package/software names.
...
* **Examples**: replaced all `\dontrun{}` with `\donttest{}` throughout the
  package, as the examples can in principle be executed (they require
  TensorFlow, but are not inherently unrunnable).
* **io.R**: removed the default `dir = "./models/swrc"` path from
  `save_swrc_model()` to comply with CRAN policy against writing to the
  user's home filespace by default; updated examples to use `tempdir()`.
