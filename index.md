...
**soilFlux** implements a physics-informed 1-D convolutional neural
network (CNN1D-PINN) for estimating the complete **soil water retention
curve** (SWRC) as a continuous function of matric potential. The
architecture and physics constraints are adapted from **Norouzi et
al. (2025)** and implemented in R via TensorFlow/Keras.

The model estimates volumetric water content (θ, m³/m³) at *any* pF
value from soil texture fractions, organic carbon, bulk density, and
depth — without discrete look-up tables.
...
1.  **`Xseq_knots`** (shape `[N, K, p+1]`) — covariates broadcast across
    `K` knot positions.
2.  **`pf_norm`** (shape `[N, 1]`) — query pF value normalised to
    `[0, 1]`.

Output:

$$\widehat{\theta}\left( \text{pF} \right) = \theta_{s} - \int_{0}^{\text{pF}}\text{softplus}\left( s(t) \right)\, dt$$

where $s(t)$ is a Conv1D output. Because `softplus > 0`, the integral is
strictly increasing, so $\widehat{\theta}$ is **strictly decreasing** —
monotonicity is guaranteed *by architecture*, not by post-processing.
...
| [`predict_theta_s()`](https://hugomachadorodrigues.github.io/soilFlux/reference/predict_theta_s.md)             | Extract modelled θs                    |
| [`evaluate_swrc()`](https://hugomachadorodrigues.github.io/soilFlux/reference/evaluate_swrc.md)                 | R², RMSE, MAE on test set              |
| [`swrc_metrics()`](https://hugomachadorodrigues.github.io/soilFlux/reference/swrc_metrics.md)                   | Regression metrics                     |
...
| [`save_swrc_model()`](https://hugomachadorodrigues.github.io/soilFlux/reference/save_swrc_model.md)             | Save weights + metadata                |
| [`load_swrc_model()`](https://hugomachadorodrigues.github.io/soilFlux/reference/load_swrc_model.md)             | Reload saved model                     |
| [`plot_swrc()`](https://hugomachadorodrigues.github.io/soilFlux/reference/plot_swrc.md)                         | SWRC curve figure                      |
| [`plot_pred_obs()`](https://hugomachadorodrigues.github.io/soilFlux/reference/plot_pred_obs.md)                 | Predicted vs. observed plot            |
| [`plot_swrc_metrics()`](https://hugomachadorodrigues.github.io/soilFlux/reference/plot_swrc_metrics.md)         | Model comparison bar chart             |
| [`plot_training_history()`](https://hugomachadorodrigues.github.io/soilFlux/reference/plot_training_history.md) | Training loss curves                   |
| [`classify_texture()`](https://hugomachadorodrigues.github.io/soilFlux/reference/classify_texture.md)           | USDA texture classification            |
| [`texture_triangle()`](https://hugomachadorodrigues.github.io/soilFlux/reference/texture_triangle.md)           | Ternary texture diagram                |
...
(2025). Physics-Informed Neural Networks for Estimating a Continuous
